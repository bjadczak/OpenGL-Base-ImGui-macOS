...
static glm::mat4 myProjectionMatrix(float fovy, float aspect, float zNear, float zFar)
{
...
{
    return myProjectionMatrix(FOVdeg, static_cast<float>(screenWidth) / static_cast<float>(screenHeight), nearPlane, farPlane);
}
